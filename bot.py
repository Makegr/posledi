...
def tts_handler(message):
    ...
    bot.send_message(user_id,
                     'Отправь что ты хочешь озвучить текстом')
...
def tts(message):
    ...
    try:
        ...
        if status_tts:
            ...
            bot.send_message(user_id, "Ошибка", reply_to_message_id=message.id)
    except Exception as e:
        logging.error(e)
        bot.send_message(message.from_user.id, "Попробуй написать другое сообщение")
...
def stt(message):
    ...
    try:
        if not message.voice:
            bot.send_message(user_id, 'Неверный тип данных. Повторите попытку, введя /stt снова')
            return
